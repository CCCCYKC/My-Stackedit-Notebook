...
## 7.`reduce`
能够对数组中的元素进行累积计算，并最终`返回一个单一的值`。它在处理数组数据时非常灵活，可以用于求和、求积、统计、转换等多种场景
...
```
-   `compareFunction`：（可选）一个用来确定元素排序顺序的函数。如果省略，则元素会被转换为字符串，并按照 UTF-16 代码单元值的顺序进行排序。
### compareFunction 的使用

`compareFunction` 接收两个参数 `a` 和 `b`，这两个参数是数组中的元素。这个函数应该返回以下值之一：

-   负数：如果 `a` 应该在 `b` 之前（顺序不变）；
    
-   零：如果 `a` 和 `b` 相等（虽然它们不一定在最终结果中相邻）；
    
-   正数：如果 `a` 应该在 `b` 之后（顺序交换）。
```js
//对数组进行数字升序排序：
[3, 1, 4, 1, 5, 9, 2].sort((a, b) => a - b);
// 结果: [1, 1, 2, 3, 4, 5, 9]

//例如，对数组进行数字降序排序：
[3, 1, 4, 1, 5, 9, 2].sort((a, b) => b - a);
// 结果: [9, 5, 4, 3, 2, 1, 1]
...
-  `slice()` 提取指定范围的元素
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTkxMjM5ODcyOCwxNzQ1NzYwNzE5LC0xNj
g2MTgxOTUwLC04NTIyNTUyNTIsLTIyNTYwODQ4MF19
-->